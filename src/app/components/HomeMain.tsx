...
export default function HomeMain({ email }: { email: string }) {
  ...
  const feature = [
    {
      ...
      desc: "Delivers real-time, intelligent responses using advanced machine learning models."
    },
    {
      title: "Multi-Platform Integration",
      desc: "Seamlessly integrate with websites, mobile apps, and messaging platforms."
    },
    {
      title: "Customizable Knowledge Base",
      desc: "Train the chatbot with your own data to provide accurate and business-specific answers."
    }
  ];
    ...
    <div className='min-h-screen bg-linear-to-br from-white to-zinc-50 text-zinc-900 overflow-x-hidden'>
      <motion.div initial={{ y: -60 }} animate={{ y: 0 }} transition={{ duration: 0.5 }} className='fixed top-0 left-0 w-full z-50 bg-white/70 backdrop-blur-xl border-b border-zinc-200'>
        <div className='max-w-7xl mx-auto px-6 h-16 flex items-center justify-between'>
          <div className='text-lg font-semibold tracking-tight'> ChatBot.<span className='text-zinc-500'>AI</span> </div>
          {email ? <div className='relative' ref={popupRef}>
            <button className='w-10 h-10 rounded-full
            bg-black text-white flex items-center justify-center font-semibold 
            hover:scale-105 transition' onClick={() => setOpen(!open)}>{firstLetter}</button>
        ...
        <div className='max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-20 items-center'>
          ...
          >
            ...
            <p className='mt-6 text-lg text-zinc-600 max-w-xl'>
              Add a Powerful ChatBot websites in a minutes.
            </p>
            <div className='mt-10 flex gap-4'>

              {email ? <button className='px-7 py-3 rounded-xl bg-black text-white font-medium
               hover:bg-zinc-800 transition disabled:opacity-60' onClick={() => navigate.push("/dashboard")}> Go to Dashboard  </button>

                :
                <button className='px-7 py-3 rounded-xl bg-black text-white font-medium
               hover:bg-zinc-800 transition disabled:opacity-60' onClick={handleLogin}>Get started</button>
          ...
          >
            <div className='rounded-2xl bg-white shadow-2xl border border-zinc-200 p-6'>
              ...
              <div className='space-y-3'>
                <div className='bg-black text-white rounded-lg px-4 py-2 text-sm ml-auto w-fit'> Yes, Cash on Delivery is avaliable</div>
                <div className='bg-zinc-100 rounded-lg px-4 py-2 text-sm w-fit'> Do you offer cash on delivery? </div>
              </div>
      ...
      <section
        id='feature'
        className='bg-zinc-50 py-28 px-6 border-t border-zinc-600'>
        ...
        <div className='max-w-6xl mx-auto'>
          ...
          >
            why Bussines choose ChatBot Ai
          </motion.h2>
          ...
          <div className='mt-16 grid grid-cols-1 md:grid-cols-3 gap-10'>
            {feature.map((f, index) => (
              <motion.div
                ...
                p-8 shadow-lg border border-zinc-200 '
              >
